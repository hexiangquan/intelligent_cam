...
static Int32 icam_send_cmd(ICamCtrlHandle hCtrl, Uint16 cmd, void *data, Int32 sndLen, Int32 rcvLen)
{
	...
	/* fill header fileds */
	hdr->magicNum = MSG_MAGIC_SEND;
	hdr->index = hCtrl->cmdIndex++;
	hdr->dataLen = sndLen - sizeof(MsgHeader);
	hdr->cmd = cmd;

	/* send msg */
	err = msg_send(hCtrl->hMsg, NULL, data, sndLen);
	if(err)
		return err;
	...
	/* validate data */
	if(hdr->cmd != cmd || hdr->magicNum != MSG_MAGIC_RESP)
		return E_TRANS;
...
Int32 icam_get_road_info(ICamCtrlHandle hCtrl, CamRoadInfo *buf)
{
	...
	ret = icam_send_cmd(hCtrl, ICAMCMD_G_RODAINFO, &msgBuf, sizeof(MsgHeader), sizeof(msgBuf));
	if(ret)
		return ret;
...
Int32 icam_set_road_info(ICamCtrlHandle hCtrl, const CamRoadInfo *info)
{
	...
	msgBuf.info = *info;
	ret = icam_send_cmd(hCtrl, ICAMCMD_S_RODAINFO, &msgBuf, sizeof(msgBuf), sizeof(msgBuf));
	if(ret)
		return ret;
...
/*****************************************************************************
 Prototype    : icam_get_drc_params
 Description  : get dynamic range params
 Input        : ICamCtrlHandle hCtrl  
                CamDRCParam *params   
 Output       : None
 Return Value : 
...
*****************************************************************************/
Int32 icam_get_drc_params(ICamCtrlHandle hCtrl, CamDRCParam *params)
{
	struct {
		MsgHeader 		hdr;
		CamDRCParam		params;
	...
	ret = icam_send_cmd(hCtrl, ICAMCMD_G_DRCPARAMS, &msgBuf, sizeof(MsgHeader), sizeof(msgBuf));
	if(ret)
		return ret;
...
/*****************************************************************************
 Prototype    : icam_set_drc_params
 Description  : set dynamic range compression params
 Input        : ICamCtrlHandle hCtrl       
                const CamDRCParam *params  
 Output       : None
 Return Value : 
 Calls        : 
 Called By    : 
...
*****************************************************************************/
Int32 icam_set_drc_params(ICamCtrlHandle hCtrl, const CamDRCParam *params)
{
	struct {
		MsgHeader 		hdr;
		CamDRCParam		params;
	...
	msgBuf.params = *params;
	ret = icam_send_cmd(hCtrl, ICAMCMD_S_DRCPARAMS, &msgBuf, sizeof(msgBuf), sizeof(msgBuf));
	if(ret)
		return ret;
...
Int32 icam_capture_ctrl(ICamCtrlHandle hCtrl, CamCapCtrl ctrl)
{
	...
	hdr.param[0] = ctrl;
	ret = icam_send_cmd(hCtrl, ICAMCMD_S_CAPEN, &hdr, sizeof(hdr), sizeof(hdr));
	if(ret)
		return ret;
