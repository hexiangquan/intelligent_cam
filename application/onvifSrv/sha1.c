...
int calc_SHA1(const char *created, const char *nonce, int noncelen, const char *password, char *hash)
{   
	   SHA1Context sha;   
	   int err;	
	   SHA1Reset(&sha);    
	   err = SHA1Input(&sha,nonce,noncelen); 
	   err = SHA1Input(&sha,created,strlen(created));     
	   err = SHA1Input(&sha,password,strlen(password));    
