...
{ (void)soap;
 ...
 // printf("soap->msgbuf=%s\n", soap->msgbuf);
// if(strstr(soap->msgbuf,"/onvif/snapshot"))
 //	return SOAP_OK;
...
{ register int err;
 ...
 {	
    ...
    int len=8192;
    int sendlen=0;
    FILE *pfile=fopen("./Release/snapshot.jpeg","rb");
    if(!pfile)
  	{
  	 printf("open ./Release/test.jpg failed\n");
	 ...
	 return err;
	  ...
	  sprintf(soap->tmpbuf, "HTTP/%s 200 OK", soap->http_version);
	   if ((err = soap->fposthdr(soap, soap->tmpbuf, NULL)))
	        return err;
	  sprintf(soap->tmpbuf, "Content-Type: image/jpeg");
	   if ((err = soap->fposthdr(soap, soap->tmpbuf, NULL)))
	        return err;
	   
	   sprintf(soap->tmpbuf, "%lu", (unsigned long)jpglen);
	   err = soap->fposthdr(soap, "Content-Length", soap->tmpbuf);
	   soap->fposthdr(soap,"\n",NULL);
	   soap_flush(soap);
	     while(1)
	   	{
		   len=fread(buf,1,8192,pfile);
		   if(len>0)
		   	soap->fsend(soap, buf,len);
		   else
		   	break;
		   sendlen+=len;
	   	}
	     fclose(pfile);
		 printf("status=%d sendlen=%d\n",status,sendlen);
		 soap->fposthdr(soap, NULL, NULL);
	     soap->error = SOAP_STOP; /* prevents further processing */
